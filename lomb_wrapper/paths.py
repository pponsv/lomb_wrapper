...
POLOIDAL_CALIBRATION_PATH = lambda: (
    f"{CONFIG_PATH}/calibration/poloidal_calibration.yaml"
)
HELICAL_CALIBRATION_PATH = lambda: f"{CONFIG_PATH}/calibration/helical_calibration.yaml"
...
def set_config_folder(folder):
    global CONFIG_PATH
    CONFIG_PATH = folder
    assert os.path.isdir(DMUSIC_PATH()), f"DMUSIC path {DMUSIC_PATH()} not found"
    assert os.path.isdir(DATA_PATH()), f"DATA path {DATA_PATH()} not found"
    assert os.path.isdir(LOMB_OUTPUT_PATH()), (
        f"LOMB path {LOMB_OUTPUT_PATH()} not found"
    )
    assert os.path.isfile(POLOIDAL_CALIBRATION_PATH()), (
        f"Poloidal calibration file {POLOIDAL_CALIBRATION_PATH()} not found"
    )
    assert os.path.isfile(HELICAL_CALIBRATION_PATH()), (
